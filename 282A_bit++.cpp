...
Input
The first line contains a single integer n (1?¡Â?n?¡Â?150) ? the number of statements in the programme.

Next n lines contain a statement each. Each statement contains exactly one operation (++ or --) and exactly one variable x (denoted as letter ¡ìX¡í). Thus, there are no empty statements. The operation and the variable can be written in any order.
