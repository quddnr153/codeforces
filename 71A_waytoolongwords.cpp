...
Thus, "localization" will be spelt as "l10n", and "internationalization¡í will be spelt as "i18n".
...
Input
The first line contains an integer n (1?¡Â?n?¡Â?100). Each of the following n lines contains one word. All the words consist of lowercase Latin letters and possess the lengths of from 1 to 100 characters.
...
void test()
{
	...
	if (input.size() <= 10) cout << input << endl;
		...
		for (int i = 0; i < 3; i++) {
			if (i == 0) cout << input[i];
			else if (i == 2) cout << input[input.size() - 1] << endl;
			else cout << input.size() - 2;
		}
	}
}
